...
for (let item in student) {
    ...
    console.log(student[item]) //values
}

let catArray = ['tabby', 'british shorthair', 'burmser', 'maine coon'];

for (cat in catArray) {
    console.log(cat);  //keys are the positions in the array
}
...
for (let letter in name) {
    if (letter == 0) {
        capitalize = name[letter].toUpperCase();
    } else {
        capitalize = capitalize + name[letter].toLowerCase();
    }
}
